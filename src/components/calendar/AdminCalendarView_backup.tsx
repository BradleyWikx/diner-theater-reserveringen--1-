...
const AdminCalendarView: React.FC<AdminCalendarViewProps> = ({
  ...
  const monthStats = useMemo(() => {
    ...
    const monthEvents = showEvents.filter(event => {
    ...
    const monthReservations = reservations.filter(r => {
      ...
      return resDate.getFullYear() === year && resDate.getMonth() === monthIndex;
    });

    const totalCapacity = monthEvents.reduce((sum, event) => sum + event.capacity, 0);
    const totalBooked = monthReservations.reduce((sum, res) => sum + res.guests, 0);
    const occupancyRate = totalCapacity > 0 ? Math.round((totalBooked / totalCapacity) * 100) : 0;

    // Calculate revenue (simplified)
    const totalRevenue = monthReservations.reduce((sum, res) => {
      const showType = config.showTypes.find(st => st.name.toLowerCase() === 'standaard');
      return sum + (res.guests * (showType?.priceStandard || 45));
    }, 0);

    return {
      totalShows: monthEvents.length,
      totalBookings: monthReservations.length,
      occupancyRate,
      totalRevenue
    };
  }, [month, showEvents, reservations, config]);
  ...
  const handleDateClick = (date: string) => {
    if (isMultiSelectMode) {
      // In multi-select mode, toggle date selection
      ...
      // In normal mode, just select the date
                  ...
                  <div className="date-details-content">
                    {selectedShow ? (
                      ...
                      <div className="empty-date">
                        ...
                        <button className="add-show-primary-btn" onClick={() => {/* Handle add show */}}>
                          ➕ Show Toevoegen
                        </button>
                      </div>
                    )}
